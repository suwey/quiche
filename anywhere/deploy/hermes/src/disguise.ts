// 伪装页面：nginx 默认页 / Cloudflare 1101 报错页。

export async function nginx(): Promise<string> {
	return `
	<!DOCTYPE html>
	<html>
	<head>
...
export async function html1101(host: string, 访问IP: string): Promise<string> {
	const now = new Date();
	const pad = (n: number): string => String(n).padStart(2, '0');
	const 格式化时间戳 = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;
