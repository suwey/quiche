...
async function 处理XHTTP请求(request, yourUUID) {
	...
	if (首包.isUDP && 首包.协议 !== atob('dHJvamFu') && 首包.port !== 53) {
		try { reader.releaseLock() } catch (e) { }
			...
			try {
				if (首包.isUDP) {
					if (首包.rawData?.byteLength) {
						if (首包.协议 === atob('dHJvamFu')) await 转发木马UDP数据(首包.rawData, xhttpBridge, 木马UDP上下文, request);
				...
				while (true) {
					...
					if (首包.isUDP) {
						if (首包.协议 === atob('dHJvamFu')) await 转发木马UDP数据(value, xhttpBridge, 木马UDP上下文, request);
...
const BLESS文本解码器 = new TextDecoder();
...
`;
	...
	const 获取代理类型 = (nodeText) => nodeText.match(/type:\s*(\w+)/)?.[1] || atob('dmxlc3M=');
	const 获取凭据值 = (nodeText, isFlowStyle) => {
		const credentialField = 获取代理类型(nodeText) === atob('dHJvamFu') ? 'password' : 'uuid';
